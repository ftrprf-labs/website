...
const TONE = [
  'Je bent de Relationship Copilot van Maculis. Toon: rustig, persoonlijk, oprecht, nooit verkoperig,',
  'nooit overdreven. Verzin geen feiten over de relatie. Je schrijft een CONCEPT dat een mens',
  'controleert, verfijnt en zelf verstuurt. Je verstuurt NOOIT zelf.',
].join(' ');
...
function mockRevise(base, instruction, channel) {
  ...
  if (/korter|beknopt|\bkort\b|to the point/.test(i)) body = firstSentences(body.replace(/\n\n(met vriendelijke groet|groet)[\s\S]*$/i, ''), 2);
...
export async function classifyIntent({ text }) {
  ...
  const raw = await getProvider().generate({ system: TONE, prompt: `Classificeer de intentie (kies uit: ${INTENTS.join(', ')}). Antwoord met alleen het label.\n\nBERICHT:\n${(text || '').slice(0, 2000)}` });
...
export async function summarizeConversation({ tenantId, conversationId }) {
  ...
  const raw = await getProvider().generate({ system: TONE, prompt: `${renderContextForModel(ctx)}\n\nVat kort samen wat er speelt en wat de afzender lijkt te vragen.` });
  return { summary: raw.slice(0, 1000), intent: mockIntent(lastInbound?.body_text), refs: ctx.refs };
}

export async function draftReply({ tenantId, conversationId, contactId, channel = 'EMAIL', instruction = null }) {
  ...
  if (!aiAvailable()) return { ok: true, body: mockDraftFromContext(ctx, channel), refs: ctx.refs };
  const prompt = [renderContextForModel(ctx), CHANNEL_HINT[channel] || '', instruction ? `INSTRUCTIE: ${instruction}` : 'Schrijf een passend conceptantwoord op het laatste bericht.'].filter(Boolean).join('\n\n');
  const raw = await getProvider().generate({ system: TONE, prompt });
  return { ok: true, body: raw.trim().slice(0, 4000), refs: ctx.refs };
}
...
export async function reviseDraft({ currentBody, subject = null, instruction, channel = 'EMAIL', tenantId = null, conversationId = null }) {
  ...
  const prompt = [
    ctx ? renderContextForModel(ctx) : null,
    CHANNEL_HINT[channel] || '',
    'Hieronder staat het HUIDIGE concept. Dit kan door een mens zijn aangepast — behoud die menselijke wijzigingen en pas alleen aan wat de instructie vraagt. Geef het volledige nieuwe concept terug, zonder toelichting.',
    `HUIDIG CONCEPT:\n${currentBody || '(leeg)'}`,
