// REVIEWINSTRUMENT: rendert het nieuwe slot van de Lens zonder backend.
import { chromium } from 'playwright';
import http from 'node:http';
import { readFile } from 'node:fs/promises';
import { join, extname } from 'node:path';
const PUBLIC='/workspace/maculis-first-five./public', PORT=4431;
...
await page.screenshot({path:process.env.SP+'/fase1-bewaarvraag.png'});
...
await page.screenshot({path:process.env.SP+'/fase1-contactvraag.png'});
await page.click('#acc-actions button');
await page.waitForTimeout(600);
...
console.log(rood?`ROOD: ${rood} van ${stappen.length}`:'FASE 1: ALLES GROEN');
